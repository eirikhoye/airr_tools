...
# Define sys args
path = sys.argv[1]
sampling_nr = int(sys.argv[2])
public_sharing = pd.read_csv('/Users/eirikhoy/Dropbox/projects/airr_comet/data/public_clones_sharing_level.csv')
...
for file in os.listdir(path):
    ...
    df[sample_name] = dataset

# load metadata
metadata = pd.read_csv('/Users/eirikhoy/Dropbox/projects/airr_comet/data/immunarch_data/metadata.txt', sep='\t')
...
nr_clones_col = []
for k, v in df.items():
    nr_tcells = v.templates.sum()
    nr_clones = len(v)
    sample_col.append(k)
    nr_tcells_col.append(nr_tcells)
    nr_clones_col.append(nr_clones)
airr_meta_dict = {
    'sample': sample_col,
...
airr_meta = pd.DataFrame.from_dict(airr_meta_dict).sort_values(by='sample_nr', ascending=True).drop('sample_nr', axis=1).reset_index(drop=True)
