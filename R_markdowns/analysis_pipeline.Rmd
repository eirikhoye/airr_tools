...
```{r message = FALSE}
# Concatenate sequence metadata from all samples and calculate T cell fraction
...
```{r message = FALSE}
# Define samples from the same patient and discard duplicate for this analysis
same_patient <- c(
...
```{r}
# Plot comparison of T cell fraction between no-NACT, short-interval and long-interval.
...
# Metadata
```{r message = FALSE}
# Reload metadata for downstream analysis
...
```{r message = FALSE}
# Load qc reports and define a sequencing coverage dataframe
...
# Load hill diversity profiles generated with changeo package
df <- readr::read_tsv(list_of_files)
df <- df %>% left_join(metadata[, c("Sample", "COMET_ID")], by=c("sample_id"="Sample")) %>%
  left_join(coverage)

# Exclude coverage < 5 and patient duplicates
...
```{r}
# Define annotations and functions required for unsupervised clustering and heatmaps
annotation <- metadata %>% 
  filter(group != 'na') %>%
...
```{r}
# Plot heatmap of diversity profiles v1
...
```{r}
# Plot heatmap of diversity profiles v2
...
```{r}
# Plot heatmap of diversity profiles v3
...
```{r}
# Plot dendrogram of diversity profile clustering
height = 2.5
...
```{r}
# Compare significance of clustering and chemo group
t <- table(t)
print(t)
...
```{r message = FALSE}
# Make a dataframe for diversity profiles
...
```{r}
# Plot both Hill diversity and evenness
leg <- get_legend(p1)
...
```{r}
# Create a dataframe of area under the curve values for each evenness profile
...
```{r}
# Plot difference in clonality for no-NACT, short-NACT and long-NACT
library(ggpubr)
